import { getToapisImageTaskStatus, isToapisRateLimitError } from "@/lib/toapis"
import {
...
export interface SyncToapisGenerationJobResult {
  job: GenerationJob
  locked: boolean
  status: "skipped" | "synced" | "retryable_error"
}

export async function syncToapisGenerationJob(
  job: GenerationJob,
  { mode = "scheduled" }: { mode?: "interactive" | "scheduled" } = {}
): Promise<SyncToapisGenerationJobResult> {
  if (!isToapisAsyncImageJob(job) || isTerminalGenerationJobStatus(job.status)) {
    return { job, locked: false, status: "skipped" }
  }

  if (mode === "interactive" && !shouldSyncToapisJobInteractively(job)) {
    return { job, locked: false, status: "skipped" }
  }
  ...
  try {
    const result = await getToapisImageTaskStatus(lockedJob.upstream_task_id)
    ...
    const partialResultError = isPartialImageResult
      ? buildPartialImageMessage({
          ...
          sourceLabel: "ToAPIs completed 结果",
    ...
    logToapisSync(status === "partial_completed" ? "partial_completed" : status === "completed" ? "completed" : "output", {
    ...
    if (!isToapisRateLimitError(message)) {
      logToapisSync("retryable_error", {
        attempts,
        error: message,
        jobId: lockedJob.id,
        upstreamTaskId: lockedJob.upstream_task_id,
...
async function cleanupReferenceUrls(urls: string[]) {
  ...
  await Promise.all(referenceUrls.map((url) => deleteReferenceImageByPublicUrl(url))).catch((error) => {
    logToapisSync("reference_cleanup_failed", {
      error: error instanceof Error ? error.message : String(error),
    })
  })
}

export function shouldSyncToapisJobNow(job: GenerationJob) {
  if (isTerminalGenerationJobStatus(job.status) || !isToapisAsyncImageJob(job)) return false
  if (!job.next_check_at) return true
  return Date.parse(job.next_check_at) <= Date.now()
}

export function shouldSyncToapisJobInteractively(job: GenerationJob) {
  if (isTerminalGenerationJobStatus(job.status) || !isToapisAsyncImageJob(job)) return false
...
function isToapisAsyncImageJob(job: GenerationJob) {
  return job.provider === "toapis" && job.type === "image" && Boolean(job.upstream_task_id)
}
...
function logToapisSync(label: string, value: unknown) {
  if (label.includes("error") || label.includes("failed") || label.includes("refund")) {
    console.warn(`[ToAPIs Sync] ${label}`, value)
    return
  }

  if (process.env.LOG_GENERATION_DEBUG !== "1") return
  console.log(`[ToAPIs Sync] ${label}`, value)
}
